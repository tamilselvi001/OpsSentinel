...
def _instruction() -> str:
    ...
    return (
        ...
        "2. Use the `category` of the closest retrieved runbook as the incident category. Call "
        "`get_category_accuracy`, `get_calibration`, and `is_novel_category` on the Arize server "
        "with that EXACT category string, so the accuracy lookup matches your history.\n"
        "3. Classify the incident (use that runbook category) and synthesize a remediation "
        "grounded ONLY in the retrieved runbook (do not invent steps outside it).\n\n"
